...
def extract_method_info(class_text):
...
| <code>handle LocalEyeAngles()</code>
...
{{note|Does nothing on players. Add the <code>max health additive bonus</code> attribute via <code>AddCustomAttribute</code> instead.}}
|-
| <code>SetModel</code>
...
A call to <code>__CollectGameEventCallbacks</code> is required to register this hook. Internally, this hook is processed the same as a game event, but with the <code>OnScriptHook</code> prefix instead of <code>OnGameEvent</code>. Therefore, see the [[Team_Fortress_2/Scripting/VScript_Examples#Listening_for_Events|example page for an explanation of how to register the hook]].
Example:
{{ExpandBox|<source lang=js>
...
| <code>float</code>
| {{bug|This value does not seem to do anything.}}
|-
...
| <code>int</code>
| {{bug|Because of a code oversight, this value is read-only. Use <code>damage_stats</code> instead which can be read and written.}}
|-
...
| <code>void DispatchAnimEvents(handle ''entity'')</code>
...
{{bug|[[prop_dynamic]] will have visual glitches with server-sided pose parameters. Use [[point_posecontroller]], or another entity like [[base_boss]] or [[funCBaseFlex]].}}
|-
...
{{bug|This method will always produce attributes with infinite duration, making the <code>duration</code> parameter non-functional.}}
...
{{note|Static attributes (i.e. attributes that exist by default on the item) cannot be removed. However, you can override their value with <code>AddAttribute</code> to effectively modify or disable them.}}
|-
...
| <code>handle GetLastWeapon()</code>
|
|-
...
| Supposed to set the player on fire, but... {{bug|Does nothing except play on-fire sound and voicelines. {{workaround|Create a [[trigger_ignite]] and EntFire <code>StartTouch</code> input on it.}}}}
|-
...
| Stuns the player for a specified duration. Move speed reduction is a fraction (0 = no reduction. 1 = total reduction, no movement). Flag combinations control the stun type and behavior.
{{note|Doesn't allow you to set <code>m_bStunEffects</code> which is responsible for "scared" particle effects and sounds.
{{workaround|Create a [[trigger_stun]] and fire <code>StartTouch</code> on it with activator being a player. Note that this trigger can only be activated once per player.}}
}}
|-
| <code>Taunt</code>
...
| <code>bool Weapon_CanUse(handle ''weapon'')</code>
|
|-
| <code>Weapon_Drop</code>
| <code>void Weapon_Drop(handle ''weapon'')</code>
| Does nothing!
|-
| <code>Weapon_DropEx</code>
| <code>void Weapon_DropEx(handle ''weapon'', Vector ''target'', Vector ''velocity'')</code>
| Does nothing!
|-
| <code>Weapon_Equip</code>
| <code>void Weapon_Equip(handle ''weapon'')</code>
| Equips a weapon in the player. This places it inside the <code>m_hMyWeapons</code> array.
|-
| <code>Weapon_SetLast</code>
| <code>void Weapon_SetLast(handle ''weapon'')</code>
|
|-
...
| <code>void Weapon_Switch(handle ''weapon'')</code>
...
| <code>handle FindVantagePoint(float ''max_distance'')</code>
...
| <code>void PressAltFireButton(float ''duration'')</code>
|
|-
| <code>PressFireButton</code>
| <code>void PressFireButton(float ''duration'')</code>
|
|-
| <code>PressSpecialFireButton</code>
| <code>void PressSpecialFireButton(float ''duration'')</code>
|
|-
...
| <code>void SetHomeArea(handle ''area'')</code>
...
{{bug|Does not fully work. Mission will always be set to 0 (no mission).}}
|-
...
{{bug|Does not work. Mission will always be set to 0 (no mission).}}
|-
...
| <code>void AddIncomingConnection(handle ''area'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#ENavDirType|ENavDirType]] ''dir'')</code>
...
| <code>Vector ComputeClosestPointInPortal(handle ''to'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#ENavDirType|ENavDirType]] ''dir'', Vector ''close_pos'')</code>
...
|-
| <code>ConnectTo</code>
| <code>void ConnectTo(handle ''area'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#ENavDirType|ENavDirType]] ''dir'')</code>
| Connect this area to given area in given direction.
|-
| <code>Contains</code>
| <code>bool Contains(handle ''area'')</code>
...
| <code>void Disconnect(handle ''area'')</code>
...
| <code>handle GetRandomAdjacentArea([[Team_Fortress_2/Scripting/Script_Functions/Constants#ENavDirType|ENavDirType]] ''dir'')</code>
...
| <code>handle FindNavAreaAlongRay(Vector ''start_pos'', Vector ''end_pos'', handle ''ignore_area'')</code>
...
| <code>bool GetNavAreasFromBuildPath(handle ''start_area'', handle ''end_area'', Vector ''goal_pos'', float ''max_path_length'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#ETFTeam|ETFTeam]] ''team'', bool ''ignore_nav_blockers'', table ''result'')</code>
...
| <code>handle GetNearestNavArea(Vector ''origin'', float ''max_distance'', bool ''check_los'', bool ''check_ground'')</code>
| Given a position in the world, return the nav area that is closest to or below that height.
|-
...
| <code>bool NavAreaBuildPath(handle ''start_area'', handle ''end_erea'', Vector ''goal_pos'', float ''max_path_length'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#ETFTeam|ETFTeam]] ''team'', bool ''ignore_nav_blockers'')</code>
| Returns true if a path exists.
|-
| <code>NavAreaTravelDistance</code>
| <code>float NavAreaTravelDistance(handle ''start_area'', handle ''end_area'', float ''max_path_length'')</code>
| Compute distance between two areas. Return -1 if can't reach 'end_area' from 'start_area'.
|-
...
| <code>int GetNumElements(handle ntity ''entity'', string ''output_name'')</code>
...
{{bug|As the <code>PreSpawnInstance</code> hook is not implemented, this callback is never fired. This can be fixed however by defining an entity table named <code>__EntityMakerResult</code> in the entity's scope.}}
...
|rowspan="2"| Returns the sum of both classes's members (XYZ).
|-
| <code>QAngle + Vector</code>
|-
| <code>Vector - Vector</code>
|rowspan="2"| <code>Vector</code>
|rowspan="2"| Returns the subtraction of both classes's members (XYZ).
|-
| <code>QAngle - Vector</code>
|-
...
|-
| <code>constructor</code>
| <code>QAngle(float ''pitch'' = 0, float ''yaw'' = 0, float ''roll'' = 0)</code>
...
|-
| <code>Assert</code>
| <code>void Assert(bool ''value'', string ''optional_message'')</code>
...
| <code>handle CreateProp(string ''classname'', Vector ''origin'', string ''model_name'', int ''activity'')</code>
| Create a prop.
|-
...
{{warning|Calling <code>RunScriptCode</code> input will add the value provided to the string pool to execute it faster in the future. If your code uses a big range of distinct values provided to this input (for example passing a float like timestamp) the string pool will eventually exceed it's limit of 65 thousand strings and crash the server (CUtlRBTree overflow). It will probably not make use of extra execution agility either. To avoid this issue you can clear the code you've provided from the string pool by creating an entity with targetname set to the corresponding code, setting <code>m_bForcePurgeFixedupStrings</code> to true and then destroying it.
{{ExpandBox|<source lang=js>
...
// Example
EntFireCodeSafe(GetListenServerHost(), "printl(" + Time() + ")", 1.0)
</source>}}
}}
|-
...
|-
| <code>GetFriction</code>
| <code>float GetFriction(handle ''player'')</code>
...
| <code>handle GetPlayerFromUserID(int ''userid'')</code>
...
{{bug|The return value is mistakenly defined as a float. It is best to use <code>.tointeger()</code> after calling this.}}
|-
...
{{bug|This is not calculated correctly and the rotation will always be relative to (0, 0, 0). As a workaround, subtract the origin from the input, call this function and then add the origin back which will perform the expected result.}}
|-
| <code>ScreenFade</code>
| <code>void ScreenFade(handle ''player'', int ''red'', int ''green'', int ''blue'', int ''alpha'', float ''fade_time'', float ''fade_hold'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#FFADE|FFADE]] ''flags'')</code>
...
{{bug|A single NULL byte will always be appended to the end of the file.}}
|-
| <code>Time</code>
...
| <code>void ClientPrint(CBasePlayer ''player'', [[Team_Fortress_2/Scripting/Script_Functions/Constants#EHudNotify|EHudNotify]] ''destination'', string ''message'')</code>
...
|-
| <code>Say</code>
| <code>void Say(CBasePlayer ''player'', string ''message'', bool ''team_only'') </code>
...
| Returns a new array of the given length where each element is set to <tt>''fill''</tt> (<tt>null</tt> by default).
|-
| <code>assert</code>
...
{{bug|Does not return correct results since it takes a 32-bit float and returns a 64-bit integer.
...
|-
| <code>newthread</code>
| <code>coroutine newthread(function ''threadfunc'')</code>
| Creates a new cooperative thread object(coroutine) and returns it .
|-
| <code>print</code>
| <code>print(string ''message'')</code>
...
| <code>void setdebughook(function ''hook_func'')</code>
| Sets the debug hook.
|-
| <code>seterrorhandler</code>
| <code>void seterrorhandler(function ''error_handler'')</code>
...
|-
| <code>suspend</code>
| <code>void suspend(any ''ret'')</code>
...
|-
| <code>writen</code>
| <code>void writen(num ''value'', char ''data_type'')</code>
|
|}
...
|-
| <code>acos</code>
| <code>acos(num ''x'')</code>
| Returns cos<sup>-1</sup>(x), -1 ≤ x ≤ 1.
|-
| <code>asin</code>
| <code>asin(num ''x'')</code>
| Returns sin<sup>-1</sup>(x), -1 ≤ x ≤ 1.
|-
| <code>atan</code>
| <code>atan(num ''x'')</code>
| Returns tan<sup>-1</sup>(x).
|-
| <code>atan2</code>
| <code>atan2(num ''y'', num ''x'')</code> (!)
...
|-
| <code>log10</code>
| <code>log10(num ''x'')</code>
| Returns log<sub>10</sub>(x).
|-
| <code>pow</code>
| <code>pow(num ''x'', num ''y'')</code>
| Returns x<sup>y</sup>.
|-
| <code>rand</code>
| <code>rand()</code>
...
|-
| <code>sqrt</code>
| <code>sqrt(num ''x'')</code>
| Returns the square root of x.
|-
| <code>srand</code>
| <code>srand(num ''seed'')</code>
...
|-
| <code>find</code>
| <code>int find(string ''search_string'', int ''start_index'' = null)</code>
